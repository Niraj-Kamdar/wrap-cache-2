...
test("save on GHES with AC available", async () => {
  ...
  const inputPath = "__tests__/test-build";
  ...
  expect(saveCacheMock).toHaveBeenCalledWith(["uuids.json"], primaryKey, {
...
test("save with reserve cache failure outputs warning", async () => {
  ...
  expect(logWarningMock).toHaveBeenCalledTimes(2); // UUIDs not found
  expect(failedMock).toHaveBeenCalledTimes(0);
});
...
test("save with server error outputs warning", async () => {
  ...
  expect(logWarningMock).toHaveBeenCalledTimes(4); // 2 UUIDs not found + 2 HTTP errors
